...
# Import the trainer
from Src.modules.model import Trainer as ModelTrainer
        ...
        for p, vs in param.items():
            ...
            for i, v in enumerate(vs):
                ...
                if self.config['model_name'] == 'model':
                    trainer = ModelTrainer(self.config, self.writer)
                else:
                    raise ValueError(f'model_name "{self.config["model_name"]}" is not supported')
